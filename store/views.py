...
def new(request):
    ...
    if form.is_valid():
        ...
        pn = form.cleaned_data['產品名稱']
        dn = form.cleaned_data['經銷商名稱']
        q = form.cleaned_data['數量']
        ddn = Dealer.objects.get(d_name=dn)
        did = ddn.d_ID
        pro = Product.objects.get(p_name=pn)
        pid = pro.p_ID
        if pid == '1':
            price = 240*q
            profit = 208*q
        ...
        Product.objects.filter(p_name=pn).update(p_inventory=p_new)
        o = Order.objects.all()
        for i in o:
            ii = i.o_ID
        oidint = int(ii)+1
        oid = str(oidint)
        Order.objects.create(o_ID=oid,o_amount=q,o_price=price,o_profit=profit,d_ID=ddn,p_ID=pro,o_date=date.today())
